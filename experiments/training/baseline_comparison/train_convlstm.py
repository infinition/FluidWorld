"""
train_convlstm.py -- ConvLSTM baseline for scaling comparison (PDE vs Transformer vs ConvLSTM).
"""
...
from fluidworld.core.convlstm_world_model import ConvLSTMWorldModel
...
def train_convlstm(args):
    ...
    log_dir = os.path.join("runs", "phase2_convlstm")
    ...
    # -- 2. ConvLSTM model --
    print(f"Init ConvLSTMWorldModel (in_channels={in_channels}, d_model={args.d_model})")
    model = ConvLSTMWorldModel(
    ...
    print(f"  Architecture: ConvLSTM hidden={args.convlstm_hidden}, kernel={args.convlstm_kernel}")
    ...
    for epoch in range(start_epoch, args.epochs + 1):
        ...
        for images in pbar:
            ...
            current_state = None  # (h, c) tuple, initialized in model
            n_steps = T_total - 1
            ...
            for t in range(n_steps):
                ...
                current_state = out["next_state"]  # (h, c) tuple
            ...
            if global_step % 500 == 0:
                ckpt_dir = Path("checkpoints/phase2_convlstm")
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="ConvLSTM Baseline - Scaling Experiment")
    ...
    parser.add_argument("--convlstm-hidden", type=int, default=64,
                        help="ConvLSTM hidden channels (adjust for param matching)")
    parser.add_argument("--convlstm-kernel", type=int, default=3,
                        help="ConvLSTM kernel size")
    ...
    train_convlstm(parser.parse_args())
