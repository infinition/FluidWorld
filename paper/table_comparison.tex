% Include this table with: \input{table_comparison}
%
...
\begin{table}[t]
    ...
    \begin{tabular}{lccc}
        ...
        \quad Position $R^2$ (spatial)$^a$    & 0.9766          & 0.9349          & \textbf{0.9665} \\
        \quad Velocity $R^2$ (after PDE)$^b$  & \textbf{0.7738} & 0.2873          & 0.5459          \\
        \quad Velocity $R^2$ (MLP probe)$^{b2}$ & 0.5600         & \textbf{0.6020} & 0.1990          \\
        \quad Direction Acc.\ (8-class)$^c$   & \textbf{38.7\%} & 22.0\%          & 33.2\%          \\
        \midrule
        \multicolumn{4}{l}{\textit{Latent-space rollout quality (cosine similarity $\uparrow$)}$^d$} \\
        \quad Step 1                          & 0.054           & \textbf{0.833}  & $-$0.020        \\
        \quad Step 10                         & 0.143           & \textbf{0.827}  & 0.041           \\
        \quad Step 19                         & $-$0.002        & \textbf{0.827}  & 0.009           \\
        \bottomrule
    \end{tabular}
    ...
    $^b$ The JEPA-style model's low \emph{linear} velocity $R^2$ reflects non-linear encoding of velocity in representation space, not failure. Cosine similarity of 0.827 at step~19 and the MLP probe result (footnote $b_2$) confirm this. \\
    $^{b2}$ With a 2-layer MLP probe, the JEPA-style model's velocity $R^2$ rises from 0.287 to \textbf{0.602} ($\Delta = +0.31$), while Pixel drops from 0.774 to 0.560 ($\Delta = -0.21$) and Random drops from 0.546 to 0.199 ($\Delta = -0.35$). The JEPA-style model is the only one whose velocity $R^2$ increases under a non-linear probe, consistent with distributed, entangled representations as described by LeCun~(2022). Linear probes underestimate its encoding of dynamics. \\
    ...
    $^d$ Latent cosine similarity measures whether the model's autonomous rollout tracks the true future. The JEPA-style model maintains $\geq 0.827$ through step~19; Pixel and Random are near zero. It produces no pixel-space predictions because no decoder was trained (by design, it operates in representation space). \\
...
\begin{table}[t]
    ...
    \begin{tabular}{lp{4.8cm}p{4.8cm}}
        ...
        Persistent state (BeliefField)
            & Proposed (LeCun 2022\footnotemark[2]), not implemented
